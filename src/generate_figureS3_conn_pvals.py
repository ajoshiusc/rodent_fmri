...
def main():
    ...
    parser.add_argument('--out', default='figureS3_conn_pvals.png', help='Output figure filename')
    ...
    # Files matching the prompt specification for connectivity profile
    # Panel A: 7-day injury vs 7-day sham (Mann-Whitney U) -> rois_affected
    # Panel B: 7-day vs 28-day getting better (Wilcoxon) -> rois_get_better
    # Panel C: 7-day vs 28-day getting worse (Wilcoxon) -> rois_get_worse
    # Like in figure 5 due to N=7 we use uncorrected unthresholded to show trends
    methods = [
        ('(A) 7d TBI vs 7d Sham\n(Mann-Whitney U)', f'{base_dir}/conn_np/{args.cmap}/rois_affected_nonparametric_uncorrected_alpha015_conn.nii.gz'),
        ('(B) 28d vs 7d Recovery\n(Wilcoxon signed-rank)', f'{base_dir}/conn_np/{args.cmap}/rois_get_better_nonparametric_uncorrected_alpha015_conn.nii.gz'),
        ('(C) 28d vs 7d Decline\n(Wilcoxon signed-rank)', f'{base_dir}/conn_np/{args.cmap}/rois_get_worse_nonparametric_uncorrected_alpha015_conn.nii.gz')
    ]
    ...
    for i, (title, path) in enumerate(methods):
        ...
        # Prevent completely blank brains rendering black if there is no significant data (especially with FDR and N=7)
