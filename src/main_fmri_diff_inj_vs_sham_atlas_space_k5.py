...
def get_fmri_diff_tpts(dir_7d, dir_28d):
    ...
    atlas_fname = '/big_disk/ajoshi/ucla_mouse_injury/ucla_injury_rats/01_study_specific_atlas_relabel_k5.nii.gz'

    flist = glob(dir_7d + '/at*.nii.gz')
    label_ids = np.arange(84*5, dtype=np.int16)
    num_time = 450
...
def plot_atlas_pval(atlas_fname, roi_ids, pval, out_fname, alpha=0.05):
    ...
    plotting.plot_stat_map(bg_img=atlas, stat_map_img=pval_vol, vmax=alpha, threshold=0.0, output_file=out_fname + '_w_k5.png',
...
def plot_atlas_var(atlas_fname, roi_ids, roi_var, out_fname):
    ...
    # plot var
    plotting.plot_stat_map(bg_img=atlas, stat_map_img=val_vol, threshold=0.0, output_file=out_fname + '_w_k5.png', draw_cross=False,
...
## Cohen's d
    ...
    plot_atlas_pval(atlas_fname, np.arange(1, num_rois+1),
                    (2-cohen_d2)/2, out_fname='rois_get_better_cohen_d', alpha=1)
    plot_atlas_pval(atlas_fname, np.arange(1, num_rois+1),
                    (2-cohen_d3)/2, out_fname='rois_get_worse_cohen_d', alpha=1)
