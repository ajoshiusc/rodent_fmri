...
# Get a list of subjects
    ...
    for i, sub in enumerate(tqdm(sublist)):
        ...
        common_atlas = '/big_disk/ajoshi/ucla_mouse_injury/transforms_12dof_affine/atlas_labels_83_standard_space_3mm.nii.gz'
        ...
        labels_7d = os.path.join(common_atlas)

        fmri_28d = os.path.join(dir_28d, 'warped3mm_' + sub_28d + '.nii.gz')
        labels_28d = os.path.join(common_atlas)
...
def plot_atlas_pval(atlas_fname, roi_ids, pval, out_fname, alpha=0.05):
    ...
    plotting.plot_stat_map(bg_img=atlas, stat_map_img=pval_vol, threshold=0.0, output_file=out_fname + '_cmn.png', draw_cross=False, annotate=True, display_mode="ortho", cut_coords=[(85-68)*1.25,(111-90)*1.25,(54-51)*1.25]) 
    plt.show()
...
def plot_atlas_var(atlas_fname, roi_ids, roi_var, out_fname):
    ...
    # plot var
    plotting.plot_stat_map(bg_img=atlas, stat_map_img=val_vol, threshold=0.0, output_file=out_fname + '_cmn.png', draw_cross=False, annotate=True, display_mode="ortho", cut_coords=[(85-68)*1.25,(111-90)*1.25,(54-51)*1.25], vmax=0.001)
    plt.show()
