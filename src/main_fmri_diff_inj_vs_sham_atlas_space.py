...
def get_fmri_diff_tpts(dir_7d, dir_28d):
    ...
    for i, sub in enumerate(tqdm(sublist)):
        ...
        labels_7d = atlas_labels

        fmri_28d = os.path.join(dir_28d, sub_28d + ".nii.gz")
        labels_28d = atlas_labels
        ...
        d, _ = brainSync(fmri_roiwise_7d_all[:, :, i], fmri_roiwise_28d_all[:, :, i])

        print(np.mean(np.matmul(d.T, fmri_roiwise_7d_all[:, :, i])))
        print(
            np.mean(
                np.matmul(fmri_roiwise_28d_all[:, :, i].T, fmri_roiwise_7d_all[:, :, i])
            )
        )

        fmri_tdiff_all[:, i] = np.linalg.norm(fmri_roiwise_7d_all[:, :, i] - d, axis=0)
